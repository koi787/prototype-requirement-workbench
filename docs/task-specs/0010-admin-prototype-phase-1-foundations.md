...
- 当前阶段：Phase 1 已完成并进入 `main`；当前文档已同步门店客户列表最新 52 列正式基线
- Phase 1 正式提交：`36fec29a40fdda4d718648fa1c512aa2f7fff001`
...
- 前 13 列严格保持第 4.2 节正式顺序，其中第 9 列为“标记无效客资”、第 10 列为“拜访次数”、第 11 列为“近7天到店次数”、第 12 列为“无效审批状态”、第 13 列为“ID”；“首次分配时间”为第 50 列、“创建时间”为第 51 列；第 52 列为“操作”。
- “标记无效客资”和“无效审批状态”的业务语义、数据字段与需求对象继续相互独立，列位置分别固定为第 9 列和第 12 列。
...
- 第 4～13 列为当前重点保护区，顺序不得变更；第 10 列固定为“拜访次数”，第 11 列固定为“近7天到店次数”。
- “标记无效客资”固定为第 9 列，“无效审批状态”固定为第 12 列，二者继续保持独立字段语义与需求对象。
...
- `rule`：`approved` 展示“是”；`null`、`pending`、`rejected` 展示“否”；与“无效审批状态”保持业务语义独立。
...
2026-08-10 经产品经理明确授权的字段顺序 hotfix 是独立于 Phase 1 的后续页面基线调整，仅允许移动 `columns.tsx` 中既有列对象并同步当前测试、Story、CHANGELOG 与本任务单；`shared/admin` 公共组件继续冻结。
...
- 禁止直接恢复旧 `StoreCustomerList.tsx`；Phase 1 已基于正式代码完成最小接回并以 `36fec29a40fdda4d718648fa1c512aa2f7fff001` 进入 `main`。
...
1. Phase 1 已从当时最新 `main` 重新实施并以 `36fec29a40fdda4d718648fa1c512aa2f7fff001` 进入正式分支；不得通过恢复 stash 重建成果。
...
- 筛选区尺寸和网格；
- 52 列及第 4.2 节完整顺序；
- “标记无效客资”第 9 列与“无效审批状态”第 12 列的位置、独立展示及状态联动；
- 行高；
- 固定列；
- 横向滚动；
- 排序；
