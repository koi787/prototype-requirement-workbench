...
- 门店客户列表 52 列字段基线（当前状态，不追溯改写 0008 历史"当时 51 列"事实）
  - 当前页面由 51 列调整为 52 列，前 3 列固定为"姓名 → 手机号 → 客资来源"；第 4～10 列为业务重点字段区（最新分配时间、预约到店时间、是否到店、是否成交、新办成交金额、标记无效客资、无效审批状态）；"首次分配时间"位于尾部第 50 列、"创建时间"第 51 列、"操作"固定为第 52 列。
...
- 门店客户列表字段顺序修正（当前状态）：将"首次分配时间"从原第 11 列移至尾部倒数区域，明确为第 50 列（紧邻"创建时间"之前）。新的前 10 列为姓名、手机号、客资来源、最新分配时间、预约到店时间、是否到店、是否成交、新办成交金额、标记无效客资、无效审批状态；尾部为最新留资时间（49）→ 首次分配时间（50）→ 创建时间（51）→ 操作（52）。`firstAssignTime` 字段名、数据逻辑与排序能力保持不变，requirement key、requirementNo SC-01-01、displayNumber 1、targetId `first-allocation-time-column` 均保持不变；总列数仍严格为 52。0010 任务单最新字段保护基线同步更新（业务重点区 4～11 调整为 4～10）。
